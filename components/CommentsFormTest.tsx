...
const CommentsFormTest = ({slug}:any) => {
  const [error, setError] = useState(false);
  const [localStorage, setLocalStorage] = useState<any>(null);
  const [showSuccessMessage, setShowSuccessMessage] = useState(false);
  const [formData, setFormData]:any = useState({ name: null, email: null, comment: null, storeData: false });
  ...
  return (
    ...
    <div className="grid grid-cols-1 gap-4 mb-4">
      <div>
        ...
        <label className="text-gray-500 cursor-pointer m-2" htmlFor="storeData"> Save my name, email in this browser for the next time I comment.</label>
      </div>
    </div>
...
export default CommentsFormTest